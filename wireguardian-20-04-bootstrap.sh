...
services:
  web:
    ...
    labels:
      ...
      # For Let's Encrypt set a valid Host
      # - "traefik.http.routers.web.rule=Host(\`wireguardian.example.com\`)"
      - "traefik.http.routers.web.rule=HostRegexp(\`{any:.+}\`)"
      - "traefik.http.routers.web.tls=true"
      # Uncomment the following line to enable Let's Encrypt Certificates
